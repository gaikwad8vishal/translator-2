...
const languages = [
  { code: "auto", name: "Detect Language" }, { code: "en", name: "English" }, { code: "hi", name: "Hindi" }, 
    ...
    <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-gray-100">
      <div className="w-full max-w-3xl bg-white shadow-lg rounded-lg p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
          <select className="p-2 border rounded" value={sourceLang} onChange={(e) => setSourceLang(e.target.value)}>
            {languages.map((lang) => (
              <option key={lang.code} value={lang.code}>{lang.name}</option>
            ))}
          </select>

          <button className="p-2 hover:bg-gray-200 rounded-full flex justify-center items-center" onClick={swapLanguages}>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
            <path stroke-linecap="round" stroke-linejoin="round" d="M7.5 21 3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5" />
          </svg>
          </button>

          <select className="p-2 border rounded" value={targetLang} onChange={(e) => setTargetLang(e.target.value)}>
            {languages.map((lang) => (
              <option key={lang.code} value={lang.code}>{lang.name}</option>
            ))}
          </select>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {/* Output Textarea */}
          <textarea
            className="w-full p-3 focus:outline-none border resize-none rounded bg-gray-100"
            rows={6}
